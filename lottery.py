...
class Lottery(sp.Contract):
    def __init__(self):
        self.init(
            ...
            tickets_available = sp.nat(7),
            max_tickets = sp.nat(7),
        )
...
def test():
    ...
    scenario += lottery.buy_ticket().run(amount = sp.tez(2), sender = akash)
    ...
    scenario += lottery.buy_ticket().run(amount = sp.tez(4), sender = amit)
    scenario += lottery.buy_ticket().run(amount = sp.tez(5), sender = tittu)

    scenario.h2("buy_ticket (failure test)")
    scenario += lottery.buy_ticket().run(amount = sp.tez(2), sender = akash, valid = False)
